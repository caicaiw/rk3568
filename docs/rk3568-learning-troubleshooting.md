...
| Toolkit2 / NumPy 安装 | 教程单独安装 NumPy 的步骤 | 跳过，直接安装配套 requirements 文件 |
...
### 安装 Toolkit2 时跳过教程中单独安装 NumPy 的步骤

教程中的 `pip install numpy==1.16.6` 这一步跳过。我们使用的 RKNN Toolkit2 2.3.2 配套 `requirements_cp38-2.3.2.txt` 对 NumPy 的版本要求为 `numpy<=1.26.4`，与旧教程指定的版本要求不同，直接安装配套依赖文件即可，无需预先单独安装 NumPy。

![迅为教程中的 NumPy 安装步骤：跳过顶部单独安装 numpy==1.16.6 的命令](images/rk3568-troubleshooting/toolkit2-old-numpy-install-guide.png)
