...
YOLOv1 将整张图片划分为 $S\times S$ 个 grid cell。一个目标的 bbox 中心落在哪个 cell，就由哪个 cell 负责该目标。
...
在网格坐标系中，每个 cell 的尺寸正是 $1\times1$。`3.22` 表示横向走过 3.22 个 cell：已经完整经过 3 个 cell，又进入列索引为 3 的 cell（从左数第 4 列）的 22%。
...
| 有目标但 bbox 不负责 | 不计算 | 不重复计算 | 按无目标项约束 $c\to0$ |
| 没有目标 | 不计算 | 不计算 | 计算，使 $c\to0$ |

类别损失不是对同一 cell 的两个 bbox 各算一次，因为它们共享类别概率。没有目标的 cell 也没有单独的“背景类”，主要通过 $c\to0$ 表达背景。
...
- $\lambda_{noobj}$ 太大：模型过于保守，容易漏检；
- $\lambda_{noobj}$ 太小：背景误报惩罚不足，容易误检。
...
每个检测头的每个 $H\times W$ 空间位置都会使用 3 个 anchor，因此该检测头产生 $3HW$ 个原始候选框。对于 `640×640` 输入，三个尺度合计：
