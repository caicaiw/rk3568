...
训练时存在真实框，可以计算 IoU 并监督 `c`：有目标的负责预测器以 IoU 为目标值，没有目标的 cell 以 0 为目标值。推理时没有真实框，IoU **不是现场计算的**，而是网络根据训练经验直接预测 `c`。
...
非负责 bbox 不计算该目标的坐标损失。训练时它不是被直接删除，只是没有被选为该目标的负责预测器。
...
| 有目标但 bbox 不负责 | 不计算 | 不重复计算 | YOLOv1 原论文的负责指示项不让它承担该目标损失 |
...
## 8. YOLOv1 的问题与 YOLOv5 的解决方式

前面已经说明了 YOLOv1 如何预测。接下来不单独罗列 YOLOv5 的新名词，而是从 YOLOv1 暴露的问题出发，理解 YOLOv5 为什么这样设计。

| YOLOv1 的做法或问题 | YOLOv5 的解决方式 |
|---|---|
| `Flatten + 全连接层` 参数量大、输入输出尺寸固定、显式空间结构被展平 | 保留二维特征图，用 `1 × 1` 卷积直接映射预测通道 |
| 只在单个 `7 × 7` 尺度检测，小目标细节容易消失 | 使用三个不同分辨率的检测头 |
| 浅层定位细节和深层语义信息缺少充分融合 | 使用 FPN + PAN 融合多尺度特征 |
| 网络加深后需要兼顾特征表达、梯度传播和计算量 | 使用 CBL、CSP 等模块组织卷积与特征路径 |
| 每个 cell 直接给出多个候选框，同一 cell 的 bbox 共享类别概率 | 每个检测位置基于多个 anchor 预测，每个 anchor 独立输出 bbox、objectness 和类别 |

![早期 YOLOv5s 的 Backbone、Neck 与 Prediction 结构](images/yolo/yolov5s-architecture.png)

*图：早期 YOLOv5s 结构示意，包括 Backbone、Neck 和三个 Prediction 分支。图中输入为 `608 × 608`，所以预测尺度是 `76 × 76`、`38 × 38` 和 `19 × 19`。本笔记只解释我们已经讨论过的全卷积预测、多尺度、FPN/PAN、CBL/CSP 与 anchor。*

### 8.1 问题一：用全连接层生成预测，参数量太大
...
YOLOv5 不再把特征图展平成一维向量，也不使用巨大的全连接层生成预测。它保留特征图的 `H × W` 空间结构，在每个检测尺度的末端使用 `1 × 1` 卷积：
...
- `N`：batch size；
- `Cin`：Neck 输出给检测头的输入特征通道数；
- `H, W`：当前检测尺度的空间尺寸；
- `A`：每个位置使用的 anchor 数量，通常为 3；
- `5+C`：每个 anchor 的 `x, y, w, h, objectness` 和 `C` 个类别概率。

`1 × 1` 卷积不改变 `H × W`，只在每个空间位置上把 `Cin` 维特征映射成需要的预测通道数。卷积核的输出通道数设置为：
...
这里的 `255` 不是 255 个 bbox，而是：

```text
3个anchor × 每个anchor的85个预测值
```

与 YOLOv1 相比，关键变化是：

```text
YOLOv1：特征图 → Flatten → 全连接层 → 7×7×30
YOLOv5：二维特征图 → 1×1卷积映射通道 → 保持H×W的预测张量
```

每个 anchor 都独立拥有 bbox、objectness 和类别概率，不再像 YOLOv1 那样由同一 cell 的多个 bbox 共享一组类别概率。

## 9. 问题二：单个检测尺度不利于兼顾大小目标
...
每个检测头的每个空间位置使用 3 个 anchor。对于 `640 × 640` 输入，三个尺度在筛选前一共产生：
...
## 10. 问题三：浅层细节与深层语义如何融合——FPN 与 PAN
...
## 11. 问题四：网络加深后如何兼顾表达能力与计算——CBL 与 CSP
...
## 12. 问题五：如何让网络更容易预测不同形状的框——Anchor
...
8. YOLOv5 不使用 YOLOv1 的 `Flatten + 全连接层` 生成预测，而是用 `1 × 1` 卷积把输入特征通道映射为预测通道，同时保留 `H × W` 空间结构。
9. YOLOv5 的 `255=3×(5+80)`，表示 3 个 anchor 各自拥有 85 个预测值，不是 255 个 bbox。
10. Anchor 是参考模板，不是最终 bbox，也不是针对每张图片临时生成的框。
11. 三个检测头各有三个 anchor，共 9 种模板，不是三个 anchor 一一对应三个检测头。
12. Anchor 解码与 Letterbox 坐标还原是两件不同的事。
